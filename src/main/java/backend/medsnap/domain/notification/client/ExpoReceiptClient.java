...
public class ExpoReceiptClient {
        ...
        try {
            ...
            if (response.statusCode() != 200) {
                ...
                throw new NotificationException(
                        ErrorCode.NOTIFICATION_SEND_FAIL,
            ...
            ReceiptResponse receiptResponse =
                    objectMapper.readValue(cleanedBody, ReceiptResponse.class);
            log.info(
                    "푸시 리시트 조회 성공: {}개 리시트",
            ...
            throw new NotificationException(
                    ErrorCode.NOTIFICATION_SEND_FAIL, "푸시 리시트 조회 중 오류 발생: " + e.getMessage());
        }
    }
    ...
    private String cleanResponse(String response) {
        ...
        if (!cleaned.equals(response)) {
            log.warn(
                    "응답에서 제어 문자 제거됨: 원본 {} bytes → 정리 후 {} bytes",
                    response.length(),
                    cleaned.length());
        }
    ...
    public static class ReceiptResponse {
        ...
        public Map<String, Receipt> getData() {
            return data;
        }

        public void setData(Map<String, Receipt> data) {
            this.data = data;
        }
